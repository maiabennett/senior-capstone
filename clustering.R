...
p1 <- DimPlot(seurat.12, reduction = "umap", group.by = "condition")
...
### NKs: CD56/NCAM1+, EOMES+
### Ts: CD3/CD3D+ (definite)
FeaturePlot(seurat.12, features = c("CD3A", "NCAM1", "EOMES", "IL7R", "PTGDR2", "KIT", "KLRB1", "GATA3"), min.cutoff = "q9")

## Assign identity to clusters
new.cluster.ids <- c("")
names(new.cluster.ids) <- levels(seurat.12)
seurat.12 <- RenameIdents(seurat.12, new.cluster.ids)
DimPlot(seurat.12, reduction = "umap", label = TRUE, pt.size = 0.5) + NoLegend()

## Visualize specific markers by condition and cell type
Idents(seurat.12) <- factor(Idents(seurat.12), levels = c(""))
markers.to.plot <- c("CD3A", "NCAM1", "EOMES", "IL7R", "PTGDR2", "KIT", "KLRB1", "GATA3")
DotPlot(seurat.12, features = markers.to.plot, cols = c("blue", "red"), dot.scale = 8, split.by = "stim") +
  RotatedAxis()

## Visualize specific genes in cowplot format by condition and cell type
theme_set(theme_cowplot())
ilcs <- subset(seurat.12, idents = c("ILC1", "ILC2", "ILC3"))
Idents(ilcs) <- "condition"
avg.ilcs <- as.data.frame(log1p(AverageExpression(ilcs, verbose = FALSE)$RNA))
avg.ilcs$gene <- rownames(avg.ilcs)

genes.to.label = c("") # Add metabolic-related genes?
p1 <- ggplot(avg.ilcs, aes(healthy, UC)) + geom_point() + ggtitle("ILCs")
